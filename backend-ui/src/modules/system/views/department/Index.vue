  ...
  <a-card :bordered="false">
    <div>
      <div class="table-operator">
        <a-button type="primary" icon="plus" @click="handleCreate(0)">添加部门</a-button>
        <a-button type="default" icon="plus" @click="handleCreate(selectKey)" v-show="selectKey">添加子部门</a-button>
      ...
      <a-row :gutter="24">
        <a-col :xs="24" :md="6" :lg="7" v-permission:department_query>
...
import {
  getDepartmentList, departmentCreate, departmentUpdate, departmentDelete
} from '@/modules/system/api/department'
...
export default {
  ...
  created () {
    ...
    this.getDepartmentList().then((res) => { this.loading = false })
  },
  methods: {
    ...
    handleEdit (item) {
      this.title = '编辑部门：' + item.title
      this.formData = setFormValue(_.cloneDeep(defaultFormData), item)
      this.visible = true
    },

    handleAdd (key) {
      if (!this.$permission('department_add')) {
    ...
    handleSearch (val) {
      this.getDepartmentList({ name: val })
    },
    ...
    async handleReload () {
      ...
      await this.getDepartmentList().then(() => { this.reloadLoading = false })
