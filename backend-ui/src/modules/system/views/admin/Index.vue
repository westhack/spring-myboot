<template>
  <a-card :bordered="false">
    ...
    <search-form-generator
      ...
      v-permission:adminListquery></search-form-generator>
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handleCreate" v-permission:admin_create>新建</a-button>
      ...
      <a-dropdown v-if="selectedRowKeys.length > 0">
        ...
        <a-menu slot="overlay">
          <a-menu-item key="delete" v-permission:admin_delete>
          ...
          <a-menu-item key="status_enable" v-permission:admin_enable>
          ...
          <a-menu-item key="status_disable" v-permission:admin_disable>
          ...
          <a-menu-item key="status_disable" v-permission:admin_disable>
            <a-popconfirm title="确定设置待审核?" okText="是" cancelText="否" @confirm="handleTableTopBarAction({ key: 'status_uncheck'})">
              <div><a-icon type="warning" /> 待审核</div>
            </a-popconfirm>
          </a-menu-item>
        </a-menu>
      </a-dropdown>
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        <a @click="handleEdit(record)" v-show="record.id !== 1 || userInfo.id === 1" v-permission:admin_update>编辑</a>
        ...
        <a-dropdown v-show="record.id !== 1">
          ...
          <a-menu slot="overlay" >
            ...
            <a-menu-item key="status_enable" v-if="record.status != status_enable" v-permission:admin_enable>
              <a-popconfirm title="确定设置正常?" okText="是" cancelText="否" @confirm="handleTableRowBarAction('status_enable', record)">
                <a-icon type="eye" /> 正常
              </a-popconfirm>
            </a-menu-item>

            <a-menu-item key="status_disable" v-if="record.status != status_disable" v-permission:admin_disable>
            ...
            <a-menu-item key="status_uncheck" v-if="record.status != status_uncheck" v-permission:admin_disable>
              <a-popconfirm title="确定设置待审核?" okText="是" cancelText="否" @confirm="handleTableRowBarAction('status_uncheck', record)">
                <a-icon type="warning" /> 待审核
              </a-popconfirm>
            </a-menu-item>
...
import { getAdminList, adminCreate, adminUpdate, adminDeleteByIds, adminResetPassword, adminChangeStatus } from '@/modules/system/api/admin'
import { roleOptions } from '@/modules/system/api/role'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return getAdminList(Object.assign(parameter, { search: vm.queryParam }))
          .then(res => {
            return res.data
          })
      },
  ...
  methods: {
    ...
    async handleResetPassword () {
      ...
      const res = await adminResetPassword({ id: this.selectedRowKeys })
      if (res.code === httpResponseCode.SUCCESS) {
