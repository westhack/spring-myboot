package com.limaopu.myboot.core.config.security.jwt;
...
public class AuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        log.info("=========> onAuthenticationSuccess " + list);
        ...
        Integer  expiresIn = tokenProperties.getSaveLoginTime() * 60 * 24;
        Map<String, Object> map = new HashMap<>(16);
        map.put("token", token);
        map.put("expiresAt", expiresIn);

        ResponseUtil.out(response, ResponseUtil.resultMap(true, 200, "登录成功", map));
    }
}
