...
public class QiniuUtil {
     ...
     * 生成上传凭证，然后准备上传
     */
    @Value("${myboot.qiniu.accessKey}")
    private String accessKey;

    @Value("${myboot.qiniu.secretKey}")
    private String secretKey;

    @Value("${myboot.qiniu.bucket}")
    private String bucket;

    @Value("${myboot.qiniu.domain}")
    private String domain;

    @Value("${myboot.qiniu.zone}")
    ...
    public String qiniuUpload(String filePath, String key) {
        ...
        try {
            ...
            throw new MyBootException("上传文件出错，请检查七牛云配置，" + r.toString());
        }
    }
